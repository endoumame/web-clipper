...
- ユースケース層 -- `suggestAndApplyTags` のユニットテスト。`TagSuggester` をモックしタグマージロジックを検証する
- インフラ層 -- `WorkersAiTagSuggester` のテスト。AI レスポンスの JSON パースや部分的成功、パース失敗のケースを検証する
- 統合 -- `clipArticle` → `waitUntil` → タグ付け完了後に記事のタグが更新されていることを確認する
